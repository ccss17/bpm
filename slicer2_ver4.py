...
class Slicer:
    ...
    def segment_tag_over_max_length(self, seg_start, seg_end, seg_index, rms_list, seg_tags, flag = 0):
        silence_tags = []
        sil_start = None
        sil_end = None
        ...
        for tag in silence_tags:
            if tag[0] - seg_start <= self.max_length and seg_end - tag[1] <= self.max_length:
                if flag == 0:
            ...
            elif tag[0] - seg_start <= self.max_length and seg_end - tag[1] > self.max_length:
                if flag == 0:
                    del seg_tags[seg_index]
                seg_tags.append((seg_start, tag[0]))
                self.segment_tag_over_max_length(tag[1], seg_end, seg_index+1, rms_list, seg_tags, 1)
                return
                ...
                if flag == 0:
                    del seg_tags[seg_index]
                seg_tags.append((seg_start, tag[0]))
                self.segment_tag_over_max_length(seg_start, tag[0], seg_index+1, rms_list, seg_tags, 1)
                return
    ...
    def slice(self, waveform):
        ...
        for i, rms in enumerate(rms_list):
            ...
            if i - silence_start >= self.min_interval: # silence 구간일 때
                ...
                if clip_length >= self.min_length: # clip의 최소 길이 보장할 때
        ...
        for i, tag in enumerate(seg_tags):
            ...
            if seg_length > self.max_length and len(tag) == 2:
                ...
                self.segment_tag_over_max_length(seg_start, seg_end, i, rms_list, seg_tags)
        ...
        if len(sil_tags) == 0:
            ...
            for i, tag in enumerate(seg_tags):
                if tag[0] == 0:
                    chunks.append(self._apply_slice(waveform, tag[0], tag[1]+1))
                else:
                    chunks.append(self._apply_slice(waveform, tag[0]-1, tag[1]+1))
            return chunks
